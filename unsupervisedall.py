...
def data_preprocessing(data):
    try:
        ...
        categories = ['Bot', 'FTP-BruteForce', 'SSH-Bruteforce', 'DDoS attacks-LOIC-HTTP', 'DDOS attack-LOIC-UDP',
                      'DDOS attack-HOIC', 'DoS attacks-Slowloris', 'DoS attacks-GoldenEye', 'DoS attacks-SlowHTTPTest',
                      'DoS attacks-Hulk', 'Infiltration', 'SQL Injection', 'Brute Force -XSS', 'Brute Force -Web']
        cat = {category: 1 for category in categories}
        cat['Benign'] = 0
        df['Label'] = df['Label'].map(cat)

        # Remove highly correlated features
        df, dropped_features = remove_highly_correlated_features(df.drop('Label', axis=1))
        df['Label'] = dff['Label']  # Re-add label after dropping correlated features
...
def main():
    try:
        ...
        report, _ = predict(best_clf, X_scaled, y_balanced)
        print(report)
    except Exception as e:
